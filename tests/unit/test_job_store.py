...
class TestJobStore:
    async def test_create_job_sets_pending_status_and_empty_stages(self, store):
        ...
        assert set(job["stages"].keys()) == {"geocode", "amenities", "score", "ai_summary"}
    ...
    async def test_update_stage_overwrites_not_appends(self, store):
        ...
        await store.update_stage("abc-123", "geocode", STAGE_RUNNING)
        job = await store.get_job("abc-123")
        assert job["stages"]["geocode"]["status"] == STAGE_RUNNING

        # Second write to the same stage must replace, not append/merge.
        await store.update_stage(
            "abc-123", "geocode", STAGE_DONE, result={"lat": 38.7, "lng": -9.1}
        )
        job = await store.get_job("abc-123")
        assert job["stages"]["geocode"] == {
            "status": STAGE_DONE,
            "result": {"lat": 38.7, "lng": -9.1},
            "error": None,
        }
        # Other stages remain untouched.
        assert job["stages"]["amenities"]["status"] == "pending"

    async def test_update_stage_records_error(self, store):
        await store.create_job("abc-123", {"address": "x"})
        await store.update_stage("abc-123", "amenities", STAGE_ERROR, error="amenity_fetch_failed: boom")

        job = await store.get_job("abc-123")
        assert job["stages"]["amenities"]["status"] == STAGE_ERROR
        assert "boom" in job["stages"]["amenities"]["error"]

    async def test_update_stage_on_missing_job_returns_none(self, store):
        result = await store.update_stage("ghost", "geocode", STAGE_DONE)
    ...
    async def test_updated_at_advances_on_write(self, store):
        ...
        await store.update_stage("abc-123", "geocode", STAGE_DONE, result={})
        job = await store.get_job("abc-123")
